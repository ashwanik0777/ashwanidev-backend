...
const ensureDacTable = async () => {
  await query(`
    CREATE TABLE IF NOT EXISTS itcell_team_members (
  ...
  const countResult = await query("SELECT COUNT(*) FROM itcell_team_members");
  if (parseInt(countResult.rows[0].count, 10) === 0) {
    ...
    await query(`
      INSERT INTO itcell_team_members (name, role, department, designation, image, email, linkedin, portfolio, bio, skills, team_type, sort_order)
      VALUES 
      ('Ashwani Kushwaha', 'Lead Full-Stack Developer', 'B.Tech CSE', 'Student Lead, IT Cell', 
       'https://res.cloudinary.com/depbzbjfu/image/upload/v1764331825/portfolio/profile/file_atjlei.jpg', 
...
const resequenceSortOrders = async (teamType) => {
  const result = await query(
    "SELECT id FROM itcell_team_members WHERE team_type = $1 ORDER BY sort_order ASC, updated_at DESC, id ASC",
    [teamType]
  );
  for (let i = 0; i < result.rows.length; i++) {
    await query("UPDATE itcell_team_members SET sort_order = $1 WHERE id = $2", [i, result.rows[i].id]);
  }
};
...
router.get("/itcell/team", async (req, res) => {
  try {
    await ensureDacTable();
    const result = await query(
      `
      SELECT * FROM itcell_team_members
    ...
    return errorResponse(
      ...
      [{ field: "itcell", message: error.message }],
      500
    );
  }
});
...
router.post("/itcell/team", authenticate, authorize(ROLES.SUPER_ADMIN), async (req, res) => {
  try {
    await ensureDacTable();
    const {
      name,
    ...
    if (finalSortOrder === undefined || finalSortOrder === null) {
      const maxResult = await query(
        "SELECT MAX(sort_order) FROM itcell_team_members WHERE team_type = $1",
        [teamType]
      );
    ...
    const result = await query(
      `
      INSERT INTO itcell_team_members 
    ...
    const updatedRow = await query("SELECT * FROM itcell_team_members WHERE id = $1", [result.rows[0].id]);
...
router.put("/itcell/team/:id", authenticate, authorize(ROLES.SUPER_ADMIN), async (req, res) => {
  try {
    await ensureDacTable();
    const { id } = req.params;
    const {
    ...
    const checkExist = await query("SELECT * FROM itcell_team_members WHERE id = $1", [id]);
    ...
    const updatedRow = await query("SELECT * FROM itcell_team_members WHERE id = $1", [id]);
...
router.delete("/itcell/team/:id", authenticate, authorize(ROLES.SUPER_ADMIN), async (req, res) => {
  try {
    await ensureDacTable();
    const { id } = req.params;
    ...
    await query("DELETE FROM itcell_team_members WHERE id = $1", [id]);
    await resequenceSortOrders(teamType);
